			...
			$( media_edit_input ).closest( 'table' ).css( 'width', '100%' ).end()
				.addClass( 'form-control' )
					.attr( 'placeholder', wpmfne_media_file_name[0] )
						.closest( 'td' )
							.append( `<div class="input-group"><div class="input-group-prepend" id="media_folder_path"><span class="input-group-text">${wpmfne_media_file_path}</span></div><div class="input-group-append"><span class="input-group-text" id="file_extension">.${wpmfne_media_file_name[1]} </span></div></div><small>${RENAME_WP_MEDIA_FILES_NAME.info_txt}</small>` );
